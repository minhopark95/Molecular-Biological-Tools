...
work=['Counting DNA length(+GC ratio)', 'Finding DNA seq location','RNA,DNA moles <-> mass','Generating complementray seq','Generating reverse seq','Translation', 'DNA, a.a. seq alignment', 'Counting a.a. seq length(+Mw)','Finding a.a. seq location', 'Converting a.a. abbreviation']
...
findingmethod=['location of the seq', 'seq of the location']
...
aligntype=['DNA seq', 'a.a. seq']
cb5=ttk.Combobox(wframe, height=2, values=aligntype, state='readonly')
cb5.set('type of seq') 
...
t.insert(END,'___ and _ mean stop codon\n\n')
t.insert(END,'DNA, a.a. alignment default value :\n')
t.insert(END,'                match score = 1\n')
t.insert(END,'                mismatch score = 0\n')
t.insert(END,'                gap penalty = -1\n')
...
def a():
    ...
    i=t.get('1.0',END)
    global seq1
    # 숫자, 불필요 문자 제거 아미노산은 _ 포함 
    if w == 0 or w==1 or w==2 or w==3 or w==4 or w==5 or w==6 and l==0:
        tem3 = [z for z in i if z.isalpha()==True]
    elif w==7 :
        tem3 = [z for z in i if z.isalpha()==True]
        if '_' in i:
            msgbox.showerror('Error','There is "_(stop)" in your sequence!')
    elif w==8 or w==9 or w==6 and l==1:
        tem3 = [z for z in i if z.isalpha()== True or z== '_']
    i=''.join(tem3)
    ...
    if w == 0 or w==1 or w==2 or w==3 or w==4 or w==5 or w==6 or w==7:
        tem3 = [z for z in j if z.isalpha()==True]
    elif w==8 or w==9:
        tem3 = [z for z in j if z.isalpha()==True or z=='_']
    j=''.join(tem3)
    ...
    if w==2 :
        error1=[]
        for i in seq1:
            if i == 'N' : error1.append(i)
        if len(error1)>0:
            msgbox.showerror('Error', "There is 'N' in your sequence! Please enter the length of your seq instead of your seq for more accurate result")

    if w==0:
        p.insert(END, 'The length of your seq\n')
        p.insert(END, '\n')
        p.insert(END, len(seq1))
        p.insert(END, '\n\n\n\n')
        p.insert(END, 'GC-ratio\n')
        p.insert(END, '\n')
        p.insert(END, gcr1)
        p.insert(END, ' %')
    ...
    if w==1:
        ...
        if l==1:
            esg=es.get()
            etg=et.get()
            if float(esg) < 0 or float(etg) < 0 :
                msgbox.showerror('Error','Start or end number cannot be negative number!')
                p.config(state=DISABLED)
            elif '.' in esg or '.' in etg :
                msgbox.showerror('Error','Start or end number cannot include decimal point!')
                p.config(state=DISABLED)
            elif float(esg)>float(etg) :
                msgbox.showerror('Error','Start number cannot be bigger than end number!')
                p.config(state=DISABLED)
            elif float(esg) > len(seq1) or float(etg) > len(seq1) :
                msgbox.showerror('Error','Start or end number cannot be bigger than seq length!')
                p.config(state=DISABLED)
            else:
    ...
    if w==3:
        ...
        if l==0:
            ...
            p.insert(END, '\n')
            p.insert(END, '\n')
            p.insert(END, 'Complementary seq\n')
            j=0
            while j<len(pseq)//5:
    ...
    if w==5:
        ...
        if l==0:
            ...
            p.insert(END, '\n')
            for b in trrframe:
                p.insert(END, 'frame'+ str(trrframe.index(b)+1)+'\n')
                tseq=[]
                i=0
                while 3*i+3<len(b):
                    tseq.append(b[3*i:3*i+3])
    ...
    if w==6:
        ...
        if len(SEQ)>10:
            for z in range(10):
                pseq=[]
                i=0
                while 10*i+10<len(SEQ[z][0]):
                    pseq.append(SEQ[z][0][10*i:10*i+10])
                    i+=1
                else: pseq.append(SEQ[z][0][10*i:len(SEQ[z][0])])
                pseq1=[]
                j=0
                while 10*j+10<len(SEQ[z][1]):
                    pseq1.append(SEQ[z][1][10*j:10*j+10])
                    j+=1
                else: pseq1.append(SEQ[z][1][10*j:len(SEQ[z][0])])
                i=0
                while 5*i+5<len(pseq):
                    p.insert(END, pseq[5*i:5*i+5])
                    p.insert(END,'\n')
                    p.insert(END, pseq1[5*i:5*i+5])
                    p.insert(END,'\n')
                    i+=1
                else: 
                    p.insert(END, pseq[5*i:len(pseq)])
                    p.insert(END,'\n')
                    p.insert(END, pseq1[5*i:len(pseq1)])
                    p.insert(END,'\n')
                    p.insert(END,'\n')
                redindex = []
                for index in range(len(SEQ[z][0])) :
                    if SEQ[z][1][index] != '-' and SEQ [z][0][index] != '-' and SEQ[z][0][index] != SEQ[z][1][index] :
                        redindex.append(index)
                for red in redindex :
                    totalrow = len(SEQ[z][0])//50 + 1
                    pseqredrow = (2*totalrow+1)*z + 4 + 2*(red // 50)
                    pseq1redrow = (2*totalrow+1)*z + 5 + 2*(red // 50)
                    column = red % 50 + (red % 50)//10
                    p.tag_add('name' + str(red), str(pseqredrow) + '.' + str(column), str(pseqredrow) + '.' + str(column+1))
                    p.tag_add('name1' + str(red), str(pseq1redrow) + '.' + str(column), str(pseq1redrow) + '.' + str(column+1))
                    p.tag_config('name' + str(red), foreground = "red")
                    p.tag_config('name1' + str(red), foreground = "red")
                    
            p.insert(END, '\n')
            p.insert(END,'There are other '+str(len(SEQ)-10)+' results')
        else:
